...
class Genetik
{
    ...
    function __construct($rs_data, $jenis_semester, $tahun_akademik, $populasi, $crossOver, $mutasi, $kode_jumat, $range_jumat, $kode_dhuhur)
    {
    ...
    {
        $i = 0;
        foreach ($this->rs_data->result() as $data) {
        ...
        $rs_RuangReguler = $this->CI->db->get_where(
            'ruang',
            [
                'jenis' => $this->TEORI
            ]
        ...
        $rs_Ruanglaboratorium = $this->CI->db->get_where(
            'ruang',
            [
                'jenis' => $this->LABORATORIUM
            ]
        );
        $i = 0;
        ...
        $rs_WaktuDosen = $this->CI->db->query(
            "SELECT
               kode_dosen,
               CONCAT_WS(
                 ':',
